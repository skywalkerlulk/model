...
$\beta=8\times10^{-7}$ m/s。温度场与水分场相互独立演化：热物性为常数，温度方程不依赖含水率；
水分扩散方程因 $D(C)$ 的含水率依赖呈非线性，其解也不反馈到温度方程。
...
见 result1.xlsx）：30 min 内表面温度由 28.00\,°C 升至 \textbf{36.79\,°C}
（100 s 时 28.18\,°C），中心升至 33.58\,°C；表面含水率由 2.5500 降至
\textbf{1.5102} kg/kg，中心保持 \textbf{2.5500} kg/kg。图~\ref{fig:preheat} 以二维热图
...
\begin{enumerate}
\item \textbf{解析基准}：谱配点温度解与 5.1.2 解析解逐点对比，最大偏差
$1.6\times10^{-4}$\,°C（图~\ref{fig:duhamel}）；
\item \textbf{Bessel 级数基准}：常扩散系数情形的谱解与级数解偏差 \textbf{$9.5\times10^{-9}$}；
\item \textbf{独立算法交叉验证}：与有限体积法求解器（400 节点，网格收敛验证后）
